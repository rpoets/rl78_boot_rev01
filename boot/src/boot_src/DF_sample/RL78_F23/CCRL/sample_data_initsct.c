...
} r_rfd_dsec_table_d[R_RFD_DSEC_MAX] = {
        {__sectop("RFD_DATA_n"),
         __secend("RFD_DATA_n"),
...
R_RFD_FAR_FUNC void Sample_Data_INITSCT(void)
{
        ...
        for (i = 0; i < R_RFD_DSEC_MAX; i++) {
                rom_p = r_rfd_dsec_table_d[i].d_rom_sectop;
                ram_p = r_rfd_dsec_table_d[i].d_ram_sectop;
                for ( ; rom_p != r_rfd_dsec_table_d[i].d_rom_secend; rom_p++, ram_p++) {
                        *ram_p = *rom_p;
                }
        }
}
