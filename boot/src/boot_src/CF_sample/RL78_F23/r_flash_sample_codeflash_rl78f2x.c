/**********************************************************************************************************************
    ...
    File Name       : r_flash_sample_codeflash_rl78f2x.c
...
#include "sample_codeflash_config.h"
#include "sample_code_initsct.h"
#include "r_rfd_common_api.h"
#include "r_rfd_rl78_code_flash_if.h" 

/**********************************************************************************************************************
  Prototype declaration
**********************************************************************************************************************/
int sample_codeflash_main(void);

/**********************************************************************************************************************
 * Function name : sample_codeflash_main
 *********************************************************************************************************************/
/**
 *  Sample main.
 *  
...
int sample_codeflash_main(void)
{
    ...
    uint8_t        l_au08_write_data[CF_WRITE_DATA_LENGTH];
    ...
    /* Create writing data */
    for (l_u16_count = 0u; l_u16_count < CF_WRITE_DATA_LENGTH; l_u16_count++)
    {
    ...
    if (SAMPLE_VALUE_U01_MASK0_1BIT == HIOSTOP)
    {
        ...
        if (R_RFD_ENUM_RET_STS_OK == l_e_rfd_status_flag)
        {
            ...
            l_e_sam_ret_value = Sample_CodeFlashControl(CF_WRITE_ADDRESS, CF_WRITE_DATA_LENGTH, l_au08_write_data);
        }
        else
        {
...
/**********************************************************************************************************************
 End of function sample_codeflash_main
 *********************************************************************************************************************/
