...
* Return Value : None
***********************************************************************************************************************/
#pragma interrupt isr_jump_handler(vect=INTP1)

#if 1
void __near isr_jump_handler(void)
{
	//if(Boot_Mode)
	if(bootpara.bl_app_mode_notice == 0xBB)
	{
		r_Config_INTC_intp1_interrupt();
	}
	else
	{
		uint16_t isr_jump_address = 0;
		void (*p_func)(void);
		
		//isr_jump_address = (uint32_t *)(0x4000 + 0x14);
		//p_func = (void (*)())(* isr_jump_address); 
		
		isr_jump_address = (0x4000 + 0x14);
		p_func = (void (*)(void))(isr_jump_address); 	
